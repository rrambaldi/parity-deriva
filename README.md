...
No broker is the default one. Six are behind it - OANDA, eToro, IG,
Capital.com, Interactive Brokers and any MetaTrader 5 broker - plus a paper
session on Twelve Data's candles, and each live session picks its own broker
and account. Instruments are whatever the chosen broker offers; the work so far
has been on DE30_EUR and EUR_USD, so not only forex. Historical candles are
kept locally in HDF5, so research runs offline and needs no broker at all:
they come from CSV exports (Dukascopy's or a broker's), from another parity
server, or from any broker that serves bid and ask bars.
...
rewritten around candles - on the OANDA v3 API first, the broker it started
with, then on the others - and moved from Python 2 to Python 3 - but the idea of an event-driven forex stack was his, and so are
the thanks.
...
* **Event bus** - `trading/engine.py` runs a thread per data source and fans
  ...
  in a backtest and live: [docs/ENGINE.md](docs/ENGINE.md).
* **Shadow execution** - a local, bar-driven broker simulator that knows
  nothing of any broker's API (`backtest/oanda.py`, named after the first
  broker it modelled). Every live session, whatever its broker, registers it
  on the engine next to the real execution handler: it sees the same orders
  and the same candles, which is what makes a step-by-step comparison
  possible. The simulations run on it too.
...
* **Data warehouse** - per-instrument HDF5 stores of bid and ask bars, filled
  from CSV exports (the settings page imports them), from another parity
  server, or from a broker's API (`data/sources.py`); `scripts/check.py`
  audits them for missing bars. `data/bulksaver.py` is the older bulk
  downloader, OANDA only.
* **Six brokers and a paper session** - `trading/providers.py` names the
  handlers each broker needs and, more to the point, declares what each
  broker *cannot* do. A wiring asks for the capabilities it needs and fails at
  startup when they are missing, so an unsupported combination is a refusal
  rather than a run that looks fine and is not. `scripts/live.py` is one
  wiring for all of them. None is a subset of another (Capital.com aside,
  which is IG under other names), which is why they are declared rather than
  ranked.
...
* **Tests** - some 1800 tests, no network access required.
...
Docker is the short way (above). By hand, on Linux, macOS or Windows:

1. **Python 3.11 or newer** (verified on 3.12), and the code:

   ```
   git clone git@github.com:rrambaldi/parity-deriva.git parity_deriva
   python3 -m venv venv
   venv/bin/pip install -r parity_deriva/requirements.txt
   ```

   The package is imported as `parity_deriva`, so the folder *holding* the
   clone goes on `PYTHONPATH`; nothing is linked into site-packages.
   `requirements-research.txt` adds IPython, SciPy and scikit-learn, for the
   notebooks only.

2. **One folder for everything that is not code** - the runs, the market data,
   the `.env` with the brokers' keys, the files to import - named by
   `PARITY_DERIVA_DATA_DIR`. It is what `/data` is in the image.

3. **Start it**, then open http://127.0.0.1:8731:

   ```
   export PYTHONPATH=$PWD PARITY_DERIVA_DATA_DIR=$HOME/parity-data
   venv/bin/python parity_deriva/scripts/web.py --host 127.0.0.1 --port 8731
   ```

   With `PARITY_DERIVA_WIZARD=1` it starts on the setup, as the image does.

4. **Market data**, whatever broker you trade on or none (settings page, market
   data tab): CSV exports imported into the stores - Dukascopy's or a
   broker's, one file for the ask and one for the bid; the archive, another
   parity server, with a token made there; or a broker's API, any broker that
   serves bid and ask bars.

5. **A broker**, only to trade - the simulations need none: its keys in
   `DATA_DIR/.env`, one `export KEY='value'` a line.

   | broker | provider | keys | details |
   | --- | --- | --- | --- |
   | OANDA | `oanda` | `OANDA_API_ACCESS_TOKEN`, `OANDA_API_ACCOUNT_ID` | [Choosing the broker](#choosing-the-broker) |
   | eToro | `etoro` | `ETORO_USER_KEY` and `ETORO_API_KEY`, or `ETORO_ACCESS_TOKEN` | [Configuring eToro](#configuring-etoro) |
   | IG | `ig` | `IG_API_KEY`, `IG_IDENTIFIER`, `IG_PASSWORD` | [Configuring IG](#configuring-ig) |
   | Capital.com | `capital` | `CAPITAL_API_KEY`, `CAPITAL_IDENTIFIER`, `CAPITAL_API_PASSWORD` | [Capital.com](#capitalcom) |
   | Interactive Brokers | `ib` | `IB_GATEWAY`, `IB_ACCOUNT_ID` | [Configuring IB](#configuring-ib) |
   | MetaTrader 5 | `mt5` | `MT5_LOGIN`, `MT5_PASSWORD`, `MT5_SERVER`, or a file | [MetaTrader 5](#metatrader-5) |
   | Twelve Data (paper) | `twelvedata` | `TWELVEDATA_API_KEY` | [The paper session](#the-paper-session-the-candle-database-and-the-skew-board) |

   Demo accounts or real money is one setting for the whole server,
   `PARITY_DERIVA_ACCOUNTS=demo|real`, and every broker follows it: see
   [A server for demo accounts, another for real money](#a-server-for-demo-accounts-another-for-real-money).

6. **Live**: a server with the trade role starts sessions from its live page,
   each on a broker and an account of its own. From a shell,
   `scripts/live.py` builds the same stack; `--dry-run` prints it with the
   broker's capabilities and touches no network:

   ```
   python scripts/live.py --dry-run --provider ig
   python scripts/live.py --provider capital --instrument EUR_USD --granularity M5
   ```

7. **Strategies**: the app's docs page says how one is written. A file uploaded
   on the settings page, or one an AI assistant sends, is a draft, backtested
   in a sandbox until enabled there. The built-in ones are in `strategy/`.

The QSForex examples are still in the tree - `examples/mac.py`,
`scripts/generate_simulated_pair.py`, `backtest/output.py`, `trading/run.py`,
`scripts/t01.py` to `t05.py` - OANDA only and not kept up: nothing above needs
them.

## Choosing the broker

A live session names its broker, and the whole stack follows: the page asks
for it, `scripts/live.py` takes `--provider`. The names are `oanda`, `etoro`,
`ig`, `capital`, `ib`, `mt5` and `twelvedata`. Only what names none - the
older scripts, `live.py` without `--provider` - falls back on

```
PROVIDER = os.environ.get('PARITY_DERIVA_PROVIDER', 'oanda')   # etc/settings.py
```

`PARITY_DERIVA_ACCOUNTS` decides practice against real (it sets `DOMAIN`) -
but each broker means something different by it. OANDA has two hosts. eToro has one host and two sets of
...
fails to authenticate rather than quietly reaching the other account;
Capital.com has two hosts as well. MetaTrader 5 reads the account's trade mode
back after the login and refuses anything but a demo, unless the server trades
real money. IB has neither: its paper account is simply a different account id, `IB_ACCOUNT_ID`
...
| | OANDA | eToro | IG | Capital.com | IB | MT5 |
| --- | --- | --- | --- | --- | --- | --- |
| bid/ask candles | yes | **no** (one series) | yes | yes | **no** (one series) | yes³ |
| candles by date range | yes | **no**, last N ≤ 1000 | yes | yes | yes | yes |
| price stream | pushed | polled | polled¹ | polled | polled¹ | polled |
| transaction stream | pushed | polled | polled¹ | polled | polled¹ | polled |
| STOP against LIMIT | different | **both `mit`** | different | different | different | different |
| which leg closed a trade | stated | **inferred** | **inferred** | **inferred** | stated² | stated |
| order expiry | `gtdTime` | **none** | `GOOD_TILL_DATE` | `goodTillDate` | **none** | a time, or ours⁴ |
| order result | in the reply | poll after | poll `/confirms` | poll `/confirms` | poll after | in the reply |
| a bracket is | one order | one order | one order | one order | **three orders** | one order |
...
³ MetaTrader's bars are bid with the bar's spread in points, so the ask comes
from the broker's own figure rather than a configured one.

⁴ `ORDER_TIME_SPECIFIED` where the symbol allows it, and our own cancel at
`gtdTime` where it does not.

Read the table by columns and no broker is a subset of another - Capital.com
aside, which is IG's API under other names. IG is the closest to OANDA and
still cannot say which leg closed a trade; MetaTrader 5 can, and needs a
terminal running under Wine; IB can say it
...
pass and stops - asking again would return the same window. Fill the
warehouse from anywhere else - CSV exports, the archive, a broker that serves
history by date - and trade on eToro.
...
IG is the closest of the other brokers to OANDA, and where it
...
HTTP, which is what OANDA, eToro, IG and Capital.com speak. The price of that
choice has to be stated plainly, because it is unlike those four in one
important way:
...
Capital.com's REST API is IG's with a handful of names changed, so there is no
fourth copy of a broker here: `data/ig.py` and `execution/ig.py` run as they
are, through `lib/capital.CapitalAPI`, which translates where a request or an
answer crosses - the key in `X-CAP-API-KEY`, `profitLevel` for IG's
`limitLevel`, an ISO `goodTillDate`, a position closed by `DELETE`. Measured on
a demo account on 2026-09-24. Unlike IG, nothing meters the history.

```
export CAPITAL_API_KEY=...        # generated in Settings > API integrations
export CAPITAL_IDENTIFIER=...     # the login e-mail
export CAPITAL_API_PASSWORD=...   # the key's own password, not the account's
export CAPITAL_ACCOUNT_ID=...     # optional; the preferred one if unset
```

EUR/USD is dealt in units, minimum and step 100: `CAPITAL_INSTRUMENTS` in
`etc/settings.py`.

## MetaTrader 5

Any broker with MetaTrader 5 - here the MetaQuotes demo and OANDA's EU arm,
OANDA TMS, which has no v20 API and trades only through MT5. The MetaTrader5
package exists for Windows only, so the terminal runs under Wine and
`scripts/mt5_bridge.sh` serves the package over rpyc, on 127.0.0.1 only;
`lib/mt5.py` talks to it. Wine, the terminals and the credential files live in
`DATA_DIR/mt5`.

```
export MT5_LOGIN=...  MT5_PASSWORD=...  MT5_SERVER=...
# or DATA_DIR/mt5/mq.txt: login, password and server as key: value lines
scripts/mt5_bridge.sh             # leave it running: one bridge per terminal
```

One terminal is one account. For another, `scripts/mt5_add_account.sh` makes a
portable terminal and prints its `MT5_TERMINALS` entry, with a bridge port of
its own; `'suffix'` there is what the broker adds to every symbol (OANDA TMS:
`EURUSD.pro`). MetaTrader reports bars and deals in the broker's clock passed
off as UTC, and `lib/mt5.py` reads the offset from the newest tick. The bridge
...
```unittest```, no network access, no broker account needed). Run it with:
