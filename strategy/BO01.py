...
#  G R R
			...
			p[1].direction()>0 and p[2].direction()<0 and p[3].direction()<0:
				...
				self.logger.debug("BO01 %s EVENT P1 %s : IN %d" % (i, p[1].time, out))
				self.num[out] += 1
...
#			return ;
	...
	def printStats(self):
		...
		self.logger.debug("BO01 %s ALL TOT %d" % ( self.pair, tot))
		if tot>0:
			for j in range(0,self.depth):
				if self.num[j]!=None:
					self.logger.debug("BO01 %s ALL %d NUM: %d PERC: %6.2f" % ( self.pair, j, self.num[j],  self.num[j] / (tot*1.0) * 100.0 ))
		...
		for h in range(self.minTime.hour,self.maxTime.hour+1):
			for j in range(0,self.depth):
				if self.numh[h][j]!=None:
					...
					self.logger.debug("BO01 %s HOUR %02d-%d NUM: %d PERC: %6.2f" % ( self.pair, h, j, self.numh[h][j], x))
		...
		for w in range(0,6):
			...
			if totw>0:
				for w in range(0,6):
					for j in range(1,self.depth+1):
						self.logger.debug("BO01 %s DAY %d-%d NUM: %d PERC: %6.2f" % ( self.pair, w, j, self.week[w][j], self.week[w][j] / (totw*1.0) * 100.0 ))
