...
#			return ;
	...
	def printStats(self):
		...
		self.logger.debug("BO03 %s ALL TOT %d" % ( self.pair, tot))
		if tot>0:
			for j in range(0,self.depth):
				if self.num[j]!=None:
					self.logger.debug("BO03 %s ALL %d NUM: %d PERC: %6.2f" % ( self.pair, j, self.num[j],  self.num[j] / (tot*1.0) * 100.0 ))
